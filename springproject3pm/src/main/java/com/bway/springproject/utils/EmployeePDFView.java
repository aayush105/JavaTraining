...
public class EmployeePDFView extends AbstractPdfView{
			...
			Document document, HttpServletRequest request)
	{
		...
		HeaderFooter footer = new HeaderFooter(new Phrase(new Date(0)+" (C) bway, Page # "), true);
	...
	protected void buildPdfDocument(
		...
		for(Employee spec : list ) {
			...
			table.addCell(spec.getEmp_fname());
			table.addCell(spec.getEmp_lname());
			table.addCell(spec.getCompanyName());
			table.addCell(String.valueOf(spec.getEmp_salary()));
		}
		//add to document
		document.add(table);
	}
